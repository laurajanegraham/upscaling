---
title: "Appendix IV: forest case study"
output: pdf_document
header-includes:
    - \usepackage{booktabs}
---
...
plot_grid(sp_plot, cov_plot, labels = c("a)", "b)"),
...
c.winrange500 <- optimise(skew.score, c(0, 20), x = forests_df$winrange500)$minimum
c.elevrange <- optimise(skew.score, c(0, 20), x = forests_df$elevrange)$minimum
...
forests_df_t <- select(forests_df, varorder) %>% 
  mutate(winrange500 = log(winrange500 + c.winrange500),
         elevrange = log(elevrange + c.elevrange),
...
figure1_df <- res_global %>% mutate(type = "mean") %>% select(-fvariable) %>%
...
forest_figure <- ggplot(figure1_df, aes(x = variable, y = coef)) + 
  geom_errorbar(aes(ymin=`2.5 %`, ymax=`97.5 %`), width=0.2, position=pd) +
...
save(forest_figure, file = "results/forests_res_figure.Rda")
```
...
## Stability under model structure change

What about changes to the model? What conclusions would we draw if we only included the LS or MW version of the elevation heterogeneity variable? 

NB Ommission of confounding variables can bias parameter estimates, so if the opposing effect of MW and LS elevation is a true effect, we cannot trust the outcome of a model that contains only one of these variables. 

### Model with only MW
```{r model_mw}
mod_mw <- glm.nb(sprich ~ elevmean + winrange500 + bio1 + I(bio1^2) + 
                   bio12 + I(bio12^2) + bio15, 
                     data = forests_df_t, na.action = na.fail)
...
mwr2 <- 1 - (mod_mw$deviance / mod_mw$null.deviance)

res_mw %>% select(fvariable, coef, `2.5 %`, `97.5 %`) %>% kable(digits=3)
```

Negative, but non-significant effect of MW Elevation (500m). Change in estimate is `r round(res_mw[2, 2] - res_global[2, 2], 2)`, (change in LCI = `r round(res_mw[2, 3] - res_global[2, 3], 2)` and UCI = `r round(res_mw[2, 4] - res_global[2, 4], 2)`). $R^2 = $`r mwr2`

### Model with only LS
```{r model_ls}
mod_ls <- glm.nb(sprich ~ elevmean + elevrange + bio1 + I(bio1^2) + 
                   bio12 + I(bio12^2) + bio15, 
                     data = forests_df_t, na.action = na.fail)

res_ls <- mod_ls %>% coef %>% enframe(name = "variable", value = "coef") %>%
  left_join(mod_ls %>% confint %>% as_tibble(rownames = "variable")) %>%
  mutate(fvariable = factor(variable, levels = coefforder, 
                            labels = gsub("\n", " ", coefflabel))) %>%
  arrange(fvariable) 

lsr2 <- 1 - (mod_ls$deviance / mod_ls$null.deviance)

res_ls %>% select(fvariable, coef, `2.5 %`, `97.5 %`) %>% kable(digits=3)
```

Still a positive and significant effect of LS Elevation, but not as strong. Change in estimate is `r round(res_mw[2, 2] - res_global[3, 2], 2)`, (change in LCI = `r round(res_mw[2, 3] - res_global[3, 3])` and UCI = `r round(res_mw[2, 4] - res_global[3, 4])`).

We would conclude that topographic heterogeneity is not important, but this may be because the two effects are opposing: we have omitted variable bias. Given the multicollinearity diagnostics above, it suggests that multicollinearity is not affecting the estimates and that this is infact a real effect. 

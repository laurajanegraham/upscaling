...
elev <- raster('../DATA/elev/eudem_100m_aggregated.tif')
...
elev_100 <- raster('../DATA/elev/eudem_100m_aggregated.tif')

# create the grid level elevation mean and variance
...
# currently only doing a 2000m radius - might need to adjust
eu_grid$win_var_2000 <- winmove_upscale(eu_grid, elev_100, 2000, 
                                        "rectangle", "var")

# stack variables
win_var_2000 <- rasterize(eu_grid, forests_r, field = eu_grid@data$win_var_2000, fun = "mean")

var_list <- lapply(list(forests_r, win_var_2000, 
                        elev_var, elev_mean, wc_bio_eu), 
                   function(x) crop(x, elev))

var_stack <- stack(var_list)

names(var_stack) <- c("sprich", "winvar2000", "elevvar", "elevmean", paste0("bio", 1:19))

writeRaster(var_stack, filename="results/forests_covariates.tif", 
...
# full figure for paper
box_figure <- plot_grid(spatial_plot, res_figure, nrow = 2, labels = c("", "c)"))
save_plot("~/Google Drive/SCALEFORES/Papers/Upscaling/figures/forest_figure.tiff", 
          box_figure, base_width = 15, base_height = 9.375, dpi = 300)
save_plot("~/Google Drive/SCALEFORES/Papers/Upscaling/figures/lores_forest_figure.png", 
          box_figure, base_width = 15, base_height = 9.375)
```

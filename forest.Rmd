...
varlabel <- c("Species richness", "MW Elevation (2km)", "LS Elevation", "Mean Elevation", 
              "Temperature", "Precipitation", "Precip. seasonality")

coefforder <- c("(Intercept)", "winvar2000", "elevvar", "elevmean", "elevmean:winvar2000", 
                "elevmean:elevvar", "bio1", "bio12", "bio15", "I(bio1^2)", "I(bio12^2)")

coefflabel <- c("(Intercept)", "MW elevation", "LS elevation", "Mean elevation", 
...
eu_grid$elevvar <- nomove_upscale(grid = eu_grid, dat = elev_100, 
                                  fn = "var", na.rm = TRUE)

eu_grid$elevmean <- nomove_upscale(grid = eu_grid, dat = elev_100, 
                                   fn = "mean", na.rm = TRUE)
...
Upscaling 100m resolution elevation data for Europe to 0.5 degree resolution using a 2km radius window took:  `r round(run_time, 2)` minutes. 
...
save(spatial_plot, file = "results/forests_spatial.Rda")
...
For each model, our sample size is n = `r nrow(forests_df_t)`. 
...
```{r global_model}
mod_global <- glm.nb(sprich ~ elevmean + winvar2000 + elevvar + elevmean:winvar2000 +
                       elevmean:elevvar + bio1 + I(bio1^2) + bio12 + I(bio12^2) + bio15, 
...
plotResiduals(forests_df_t$winvar2000, 
              simulationOutput$scaledResiduals, 
              xlab = "MW elevation (2km)")

plotResiduals(forests_df_t$elevvar, 
              simulationOutput$scaledResiduals, 
              xlab = "LS elevation")
...
res_mmi %>% select(fvariable, coef, `2.5 %`, `97.5 %`, importance) %>% kable(digits = 3)
```

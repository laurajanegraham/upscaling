...
# labelling
varorder <- c("index10", "winshannon1000", "lsshannon", "habitat", "urban", "bio1")

varlabel <- c("Abundance (2010 Atlas)", "MW Shannon\n(1km)", "LS Shannon", 
              "Forest %", "Urban %", "Temperature")

coefforder <- c("(Intercept)", "winshannon1000", "lsshannon", "habitat", 
               "winshannon1000:bio1", "lsshannon:bio1",
               "urban", "bio1")
...
```{r}
mod_global <- lm(index10logit ~ winshannon1000 + lsshannon + habitat + urban + bio1 +
                   bio1:lsshannon + bio1:winshannon1000, 
...
mod_mw <- lm(index10logit ~ urban + winshannon1000 * bio1, 
                 data = jay_df_t, na.action = na.fail)
...
pred_vals <- bind_rows(
  ...
  make_predictions(mod_mw, pred = "bio1", modx = "winshannon1000", interval = TRUE)$predicted %>% 
    select(index10logit, ymax, ymin, modx_group, pred = bio1) %>% 
    mutate(covariate = "bio1", modx = "winshannon1000"),
  make_predictions(mod_mw, pred = "winshannon1000", modx = "bio1", interval = TRUE)$predicted %>%
    select(index10logit, ymax, ymin, modx_group, pred = winshannon1000) %>% 
    mutate(covariate = "winshannon1000", modx = "bio1")) %>% 
  as.tibble %>% 
...
# plot the results
ggplot(pred_vals %>% filter(covariate == "winshannon1000"), 

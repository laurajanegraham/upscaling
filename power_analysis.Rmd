...
df_mod <- df %>% group_by(n, r, rep) %>% 
  ...
  unnest() %>% group_by(n, r, model, term) %>% 
  summarise(estimate = mean(estimate), std.error = mean(std.error))
```

## Results

In all cases, the red dashed line at the true value, black dashed line at 0 (not visible if true value is 0). 
...
mod1 <- filter(df_mod, model == "mod1") %>% 
  inner_join(data.frame(truth = c(0, 0.5, 0), term = c("(Intercept)", "x1", "x2")))

ggplot(mod1, aes(x = r, y = estimate)) + 
  geom_point() + 
  geom_errorbar(aes(ymin = estimate - 1.96*std.error, ymax = estimate + 1.96*std.error)) +
  facet_grid(term ~ n) +
  geom_hline(yintercept = 0, linetype = "dotted") + 
  geom_hline(aes(yintercept = truth), colour = "red", linetype = "dotted")
...
```{r}
mod2 <- filter(df_mod, model == "mod2") %>% 
  inner_join(data.frame(truth = c(0, 0.5, 0.5), term = c("(Intercept)", "x1", "x2")))

ggplot(mod2, aes(x = r, y = estimate)) + 
  geom_point() + 
  geom_errorbar(aes(ymin = estimate - 1.96*std.error, ymax = estimate + 1.96*std.error)) +
  facet_grid(term ~ n) + 
  geom_hline(aes(yintercept = truth), colour = "red", linetype = "dotted") + 
...
```{r}
mod3 <- filter(df_mod, model == "mod3") %>% 
  inner_join(data.frame(truth = c(0, 0.5, -0.5), term = c("(Intercept)", "x1", "x2")))

ggplot(mod3, aes(x = r, y = estimate)) + 
  geom_point() + 
  geom_errorbar(aes(ymin = estimate - 1.96*std.error, ymax = estimate + 1.96*std.error)) +
  facet_grid(term ~ n) + 
  geom_hline(aes(yintercept = truth), colour = "red", linetype = "dotted") + 
...
```{r}
mod4 <- filter(df_mod, model == "mod4") %>% 
  inner_join(data.frame(truth = c(0, 0.5, 0, 0.5), term = c("(Intercept)", "x1", "x2", "x3")))

ggplot(mod4, aes(x = r, y = estimate)) + 
  geom_point() + 
  geom_errorbar(aes(ymin = estimate - 1.96*std.error, ymax = estimate + 1.96*std.error)) +
  facet_grid(term ~ n) + 
  geom_hline(aes(yintercept = truth), colour = "red", linetype = "dotted") + 
...
```{r}
mod5 <- filter(df_mod, model == "mod5") %>% 
  inner_join(data.frame(truth = c(0, 0.5, 0.5, 0.5), term = c("(Intercept)", "x1", "x2", "x3")))

ggplot(mod5, aes(x = r, y = estimate)) + 
  geom_point() + 
  geom_errorbar(aes(ymin = estimate - 1.96*std.error, ymax = estimate + 1.96*std.error)) +
  facet_grid(term ~ n) + 
  geom_hline(aes(yintercept = truth), colour = "red", linetype = "dotted") + 
...
```{r}
mod6 <- filter(df_mod, model == "mod6") %>% 
  inner_join(data.frame(truth = c(0, 0.5, -0.5, 0.5), term = c("(Intercept)", "x1", "x2", "x3")))

ggplot(mod6, aes(x = r, y = estimate)) + 
  geom_point() + 
  geom_errorbar(aes(ymin = estimate - 1.96*std.error, ymax = estimate + 1.96*std.error)) +
  facet_grid(term ~ n) + 
  geom_hline(aes(yintercept = truth), colour = "red", linetype = "dotted") + 

...
NB I have checked and the missing grid cells are missing in the data from BTO: this is not a problem for now, but will need to follow up (and possibly rerun the analysis with the additional cells). 
...
Temperature range         158.000    229.000
Precipitation             538.000   2041.000
Precip. seasonality         7.000     32.000
...
I have included temperature range, precipitation and precipitation seasonaility in here to check the correlations - they are not particularly correlated, so I will leave out of the model but check for patterns in their plots against model residuals. 

Based on distributions (and some initial modeling not shown) need to transform the temperature, precipitation, habitat and urban percentage variables:
...
![](jays_files/figure-html/global_validation-1.png)<!-- -->![](jays_files/figure-html/global_validation-2.png)<!-- -->![](jays_files/figure-html/global_validation-3.png)<!-- -->![](jays_files/figure-html/global_validation-4.png)<!-- -->![](jays_files/figure-html/global_validation-5.png)<!-- -->![](jays_files/figure-html/global_validation-6.png)<!-- -->![](jays_files/figure-html/global_validation-7.png)<!-- -->![](jays_files/figure-html/global_validation-8.png)<!-- -->![](jays_files/figure-html/global_validation-9.png)<!-- -->![](jays_files/figure-html/global_validation-10.png)<!-- -->


It's quite bad. I think this is to do with using the normal distribution (we have negative predicted values, and no predicted values higher than 0.6605497). There are no issues with the predictor variables as far as I can tell. Need to work out the right distribution for this and check again (see `binomial` branch on git repo). Will continue with these results for now, but potentially revisit the binomial results.  

NB there are no patterns in the residuals with the left out variables, so continue to leave out. 
...
```
## Start:  AIC=-6570.94
## index10 ~ habitat + winshannon + lsshannon + urban + bio1 + habitat:lsshannon + 
##     habitat:winshannon
## 

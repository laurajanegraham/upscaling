...
varorder <- c("index90", "index10", 
                  ...
                  "habitat", "urban", "bio1", "bio7", "bio12", "bio15")

varlabel <- c("Abundance (1990 Atlas)", "Abundance (2010 Atlas)", "MW Shannon", "LS Shannon", 
              "Forest %", "Urban %", "Temperature", "Temperature range", "Precipitation", "Precip. seasonality")
...
coefforder_int <- c("(Intercept)", "winshannon", "lsshannon", "habitat", 
               ...
               "urban", "bio1", "bio7", "bio12", "bio15")

coefflabel_int <- c("Intercept", "MW Shannon", "LS Shannon", "Forest %", 
                    "Forest % : MW Shannon", "Forest % : LS Shannon", 
                    "Urban %", "Temperature", "Temperature range", "Precipitation", "Precip. seasonality")
...
c.bio12 <- optimise(skew.score, c(0, 20), x = jay_df$bio12)$minimum
...
# the inbuild scale function stores the mean and sd used to scale, so we need to convert to vec
...
```{r global_model}
mod_global <- lm(index10 ~ habitat + winshannon + lsshannon + urban + bio1 + habitat:lsshannon + habitat:winshannon, data = jay_df_t, na.action = na.fail)
...
ggplot(jay_df_t, aes(x = winshannon, y = res_global)) + geom_point() + 
  geom_hline(yintercept = 0)

ggplot(jay_df_t, aes(x = lsshannon, y = res_global)) + geom_point() + 
  geom_hline(yintercept = 0)

ggplot(jay_df_t, aes(x = habitat, y = res_global)) + geom_point() + 
  geom_hline(yintercept = 0)

ggplot(jay_df_t, aes(x = urban, y = res_global)) + geom_point() +
  geom_hline(yintercept = 0)

ggplot(jay_df_t, aes(x = bio1, y = res_global)) + geom_point() +
  geom_hline(yintercept = 0)

# and the ones I left out
ggplot(jay_df_t, aes(x = bio7, y = res_global)) + geom_point() +
  geom_hline(yintercept = 0)

ggplot(jay_df_t, aes(x = bio12, y = res_global)) + geom_point() +
  geom_hline(yintercept = 0)

ggplot(jay_df_t, aes(x = bio15, y = res_global)) + geom_point() +
  geom_hline(yintercept = 0)
```


It's quite bad. I think this is to do with using the normal distribution (we have negative predicted values, and no predicted values higher than `r max(jay_df_t$fit_global)`). There are no issues with the predictor variables as far as I can tell. Need to work out the right distribution for this and check again (see `binomial` branch on git repo). Will continue with these results for now, but potentially revisit the binomial results.  
...
```{r simple_validation, fig.height = 8}
...
save_plot("figures/jay_results_figure_gaussian.png", res_figure, 

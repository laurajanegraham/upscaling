...
mod_global <- glm(index10 ~ habitat + winshannon + lsshannon + urban + tavg + habitat:lsshannon + habitat:winshannon, family = "binomial", data = jay_df_t, na.action = na.fail)
...
Binomial model looks a lot better than the normal one did, although there are still no fitted values higher than `r round(max(mod_global$fitted.values), 2)`. Overall it seems like it's underpredicting high values and overpredicting low. 

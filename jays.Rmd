...
First we need to load in and spatialise the Jay data. These were provided by Simon Gillings at the BTO.  
```{r bto_data}
jays <- read_sf("../DATA/bng/10km_grid_region.shp") %>% 
  rename(grid = TILE_NAME) %>% 
  inner_join(read_csv("../DATA/bto_jays/extract.csv") %>% 
               rename(grid = tenkm, index_1990 = index90, index_2010 = index10)) %>%
  select(index_1990, index_2010, geometry) %>%
  filter(!is.na(geometry))

plot(jays)
```

The data for which we want to use the upscaling approach on is lcm2015. We will use the moving window to upscale diversity of the two used habitats: Broadleaf and Coniferous forest (LCM codes 1, 2). We will calculate Shannon Diversity on just these two habitats. We will also calculate amount at the 10km scale. 
...
# shannon without moving window - move this into the winmover package at some point
...
##### BELOW HERE IS THE FOREST STUFF FOR DELETING ONCE I'VE TAKEN WHAT I NEED FROM IT ####### 
```{r projection}
